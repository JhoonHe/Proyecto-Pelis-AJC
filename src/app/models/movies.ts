const movies = [
  {
    id: "1000",
    titulo: "El Padrino",
    anioLanzamiento: 1972,
    descripcion: "La saga de una familia de mafiosos liderada por Don Vito Corleone y su hijo Michael.",
    tipo: "pelicula",
    url: "https://ca-times.brightspotcdn.com/dims4/default/2314d53/2147483647/strip/true/crop/2093x3000+0+0/resize/1200x1720!/quality/80/?url=https%3A%2F%2Fcalifornia-times-brightspot.s3.amazonaws.com%2F47%2F95%2Fcbf137037b833a53a724fde3f53e%2Fcfdj8emgq7504ddbn1ibedbziqes-ufxmz-ypi0gordml8oigvx-qxnfyjdexxje48b9jsnhzr2hbbtg9-5bm2ewo-sfvcb9svtpufonjqohy-raixysjlgk7fagt3voiify9grgz0pkj-sysmn1pfblqffyiedjz8nszvhrheowg2imuyrmeklodsplgwx3wmznmclm-gx-cvc7m0lf7h7crd5meu1oztvsd9po-e1pldrzmjwobcwk9h7avlf0y4ew56xkel9filhfyidkf9rpndq4ic7-snlvnin85poimejwzyp6rq-q8qsqvmlvl2xaq3rsy-7axvtxqcpqinvjubaglw5risa6bqkfrpqowvdt29rhgoxnbsbi3dnwzldfjuimuizsg2hody3cu-y1kgk"
  },
  {
    id: "1001",
    titulo: "Volver al Futuro",
    anioLanzamiento: 1985,
    descripcion: "Un joven viaja en el tiempo a través de un DeLorean modificado por su amigo Doc Brown.",
    tipo: "pelicula",
    url: "https://www.tematika.com/media/catalog/Ilhsa/Imagenes/638724.jpg"
  },
  {
    id: "1002",
    titulo: "Titanic",
    anioLanzamiento: 1997,
    descripcion: "El romance épico entre Jack y Rose a bordo del famoso y trágico RMS Titanic.",
    tipo: "pelicula",
    url: "https://www.abc.es/media/peliculas/000/044/428/titanic-1.jpg"
  },
  {
    id: "1003",
    titulo: "El Señor de los Anillos: La Comunidad del Anillo",
    anioLanzamiento: 2001,
    descripcion: "Un hobbit llamado Frodo emprende una aventura para destruir un anillo malévolo.",
    tipo: "pelicula",
    url: "https://cdn.hobbyconsolas.com/sites/navi.axelspringer.es/public/media/image/2016/12/senor-anillos-retorno-rey.jpg"
  },
  {
    id: "1004",
    titulo: "La La Land",
    anioLanzamiento: 2016,
    descripcion: "Un pianista y una actriz se enamoran mientras persiguen sus sueños en Los Ángeles.",
    tipo: "pelicula",
    url: "https://pics.filmaffinity.com/la_la_land-262021831-mmed.jpg"
  },
  {
    id: "1005",
    titulo: "Avengers: Endgame",
    anioLanzamiento: 2019,
    descripcion: "Los héroes del universo Marvel se unen para enfrentar al temible Thanos.",
    tipo: "pelicula",
    url: "https://lumiere-a.akamaihd.net/v1/images/690x0w_f1b0509a.jpeg?region=0%2C0%2C690%2C1035"
  },
  {
    id: "1006",
    titulo: "Barbie",
    anioLanzamiento: 2023,
    descripcion: "Una muñeca que vive en 'Barbieland' es expulsada al mundo real por no ser lo suficientemente perfecta. Basada en la popular muñeca Barbie.",
    tipo: "pelicula",
    url: "https://cuevana3.eu/_next/image?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Foriginal%2FfNtqD4BTFj0Bgo9lyoAtmNFzxHN.jpg&w=256&q=75",
    genero: ["Comedia", "Aventura", "Fantasía"],
    duracion: "1h 40min",
    calificacion: 7.8
  },
  {
    id: "1007",
    titulo: "Proyecto Extracción",
    anioLanzamiento: 2023,
    descripcion: "Dos veteranos de guerra son asignados con la misión de escoltar a un grupo de civiles en la denominada 'autopista de la muerte' en Baghdad, para que lleguen sanos y salvos a la 'Zona Verde', el emplazamiento de seguridad de la zona.",
    tipo: "pelicula",
    url: "https://cuevana3.eu/_next/image?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Foriginal%2FwqwDKolEd9lhrPGWwqTCpwJc2aT.jpg&w=256&q=75",
    genero: ["Acción", "Aventura", "Suspenso"],
    duracion: "1h 43min",
    calificacion: 7.3
  },
  {
    id: "1008",
    titulo: "Misión de rescate",
    anioLanzamiento: 2020,
    descripcion: "Tyler Rake, un intrépido mercenario que ofrece sus servicios en el mercado negro, emprende una peligrosa misión cuando es contratado para rescatar al hijo secuestrado de un señor del crimen de Bombay.",
    tipo: "pelicula",
    url: "https://cuevana3.eu/_next/image?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Foriginal%2FcNu6VozUfD9jjQUjGuGBRoMlpYA.jpg&w=256&q=75",
    genero: ["Acción", "Suspenso"],
    duracion: "1h 56min",
    calificacion: 7.4
  },
  {
    id: "1009",
    titulo: "Misión de rescate",
    anioLanzamiento: 2020,
    descripcion: "Tyler Rake, un intrépido mercenario que ofrece sus servicios en el mercado negro, emprende una peligrosa misión cuando es contratado para rescatar al hijo secuestrado de un señor del crimen de Bombay.",
    tipo: "pelicula",
    url: "https://cuevana3.eu/_next/image?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Foriginal%2FcNu6VozUfD9jjQUjGuGBRoMlpYA.jpg&w=256&q=75",
    genero: ["Acción", "Suspenso"],
    duracion: "1h 56min",
    calificacion: 7.4
  },
];

export { movies }